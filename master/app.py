...
class PredictionService(object):
    ...
    def get_model(cls):
        if cls.xgb_model is None:
            cls.xgb_model = load_model()
        return cls.xgb_model

    @classmethod
    def predict(cls, input):
        xgb_model = cls.get_model()
        return xgb_model.predict(input)

def load_model():
    # Load XGBoost model
    model = xgb.Booster({'nthread': 4})  # init model
    model.load_model(os.path.join(model_path, 'model.model'))  # load data
    return model
