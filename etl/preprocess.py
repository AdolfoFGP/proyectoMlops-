...
def split_data(df, train_percent=0.8, validate_percent=0.16, seed=123):
    np.random.seed(seed)
...
object_key = os.path.join(args['S3_INPUT_KEY_PREFIX'], 'creditcard.csv')
...
for file_name, partition_name in datasets:
    if file_name == 'test': #
        ...
        np.savetxt(
            ...
            partition_name,
            delimiter=',',
            header="Class,Time,V1,V2,V3,V4,V5,V6,V7,V8,V9,V10,V11,V12,V13,V14,V15,V16,V17,V18,V19,V20,V21,V22,V23,V24,V25,V26,V27,V28,Amount"
        )
    ...
    #elif file_name == 'validate':  # Agrega la lógica para el conjunto de datos de validación
       # print("Writing {} data ...\n".format(file_name))
       # np.savetxt(
        #    file_name+'.csv',
        #    partition_name,
        #    delimiter=',')
       # boto3.Session().resource('s3').Bucket(args['S3_OUTPUT_BUCKET']).Object(os.path.join#(args['S3_OUTPUT_KEY_PREFIX'], 'validating', file_name+'.csv')).upload_file(file_name+'.csv')
    else: # hasta ahi
        print("Writing {} data ...\n".format(file_name))
        np.savetxt(file_name+'.csv', partition_name, delimiter=',')
